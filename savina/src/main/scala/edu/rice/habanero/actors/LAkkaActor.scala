...
import akka.lacasa.actor.{BaseActor, ActorRef, ActorSystem, Safe}
import lacasa.Box
 ...
 */
abstract class LAkkaActor[MsgType](implicit val tag: scala.reflect.ClassTag[MsgType], implicit val safe: lacasa.Safe[MsgType])
  extends BaseActor {
  ...
  final def exit() = {
    ...
    if (success) {
      ...
      context.stop(self)
      onPostExit()
      LAkkaActorState.actorLatch.countDown()
    }
  }
...
protected class StartLAkkaActorMessage(promise: Promise[Boolean]) extends Safe {
  def await() {
...
object LAkkaActorState {
  ...
  def startActor(actorRef: ActorRef) {
    ...
    LAkkaActorState.actorLatch.updateCount()

    val promise = Promise[Boolean]()
    val message: StartLAkkaActorMessage = new StartLAkkaActorMessage(promise)
    actorRef ! message
