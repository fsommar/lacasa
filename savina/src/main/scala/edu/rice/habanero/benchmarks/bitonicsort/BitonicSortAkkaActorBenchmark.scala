...
object BitonicSortAkkaActorBenchmark {
  ...
  private final class BitonicSortAkkaActorBenchmark extends Benchmark {
    ...
    def runIteration() {
      ...
      ActorState.startActor(sourceActor)
  ...
  private class MergeStageActor(P: Int, N: Int, nextActor: ActorRef) extends Actor[Message] {
    ...
    val forwardActor = {
      ...
      while (i >= 1) {
        ...
        ActorState.startActor(tempActor)
        loopActor = tempActor
