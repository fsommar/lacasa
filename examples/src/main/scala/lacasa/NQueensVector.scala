...
object NQueensVector {

  def main(args: Array[String]) {
    val numWorkers: Int = NQueensVectorConfig.NUM_WORKERS
    val priorities: Int = NQueensVectorConfig.PRIORITIES
    ...
    val expSolution = NQueensVectorConfig.SOLUTIONS(NQueensVectorConfig.SIZE - 1)
    val actSolution = Master.resultCounter
    val solutionsLimit = NQueensVectorConfig.SOLUTIONS_LIMIT
  ...
  private class Master(numWorkers: Int, priorities: Int) extends Actor[Message] {
    ...
    private val solutionsLimit = NQueensVectorConfig.SOLUTIONS_LIMIT
    ...
    sendWork(
      WorkMessage(
        NQueensVectorConfig.priority(priorities),
        Vector(0),
        0))
  ...
  private class Worker(master: ActorRef, id: Int) extends Actor[Message] {
    ...
    private final val threshold: Int = NQueensVectorConfig.THRESHOLD
    private final val size: Int = NQueensVectorConfig.SIZE
    ...
    def nqueensKernelPar(workMessage: WorkMessage) {
      ...
      (0 until size)
        ...
        .filter { NQueensVectorConfig.boardValid(newDepth, _) }
        .foreach { master ! WorkMessage(NQueensVectorConfig.priority(newPriority), _, newDepth) }
    ...
    def nqueensKernelSeq(a: Vector[Int], depth: Int) {
      if (size == depth) {
        ...
        for (i <- 0 until size) {
          ...
          if (NQueensVectorConfig.boardValid(depth + 1, current)) {
